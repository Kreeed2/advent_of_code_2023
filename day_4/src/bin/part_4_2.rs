...
fn part_1(input: &str) -> u32 {    
    let mut card_amount: Vec<u32> = vec![1; input.lines().count()];

    for (pos, line) in input.lines().enumerate() {
        let wins = resolve_line(line) as usize;

        for i in pos..pos+wins {
            card_amount[i+1] += 1 * card_amount[pos]
        }
    }
    
    card_amount.into_iter().sum()
}
